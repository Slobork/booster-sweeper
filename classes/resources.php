<?php
	...
	if ( ! class_exists( 'Booster__Sweeper__Resources' ) ) {
		...
		class Booster__Sweeper__Resources {
				...
				if ( empty( $test_mode ) || $test_mode_on === true ) {
					...
					if ( ! empty($styles) && is_array($styles) ) {
						...
						foreach ( $styles as $key => $handle ) {
							// even though the $handle isn't coming from the user text input (it's the predefined selection), lets trim it for certainty
			...
			/**
			 * Have to set specific archive check coz Woo's shop page makes a problem. This is coz shop page is considered archive,
			 * but doesn't store term meta and is throwing an error notice. ( update_term_meta is used to update the resouces available in other functions bellow )
			 * Neither is_archive(), is_post_type_archive() or is_tax() was a solution, so had to make this specific check for the is_shop.
			 */
			 ...
			 *
			 * @return array The lists of frontend_styles, frontend_scripts, backend_styles, backend_scripts
			 */
			public static function prepare_resources_list_single() {
				...
				if ( ! isset($_GET['post'] ) ) {
					...
					// make sure it returns something, i.e. '' - empty string, otherwise it will produce php error on these pages
